...
import { tasks, formatPrice, formatTime, formatDate, type TaskSelection } from "@/lib/tasks-data"
...
export function Summary({ selections, totals, startDate, onStartDateChange, onBack }: SummaryProps) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <header className="sticky top-0 z-10 bg-background border-b border-border px-4 py-4 max-w-[600px] mx-auto w-full">
        <button onClick={onBack} className="flex items-center gap-2 text-white hover:opacity-80 transition-opacity">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 max-w-[600px] mx-auto w-full">
        ...
        <div className="space-y-4 mb-6">
          {essentialTasks.map((task) => (
            <SummaryItem key={task.id} name={task.name} price={task.price} quantity={selections[task.id]?.quantity} />
          ))}
          ...
          {userSelectedTasks.map((task) => (
            <SummaryItem
              ...
              name={task.name}
              price={task.price}
              quantity={task.component === "Quantity" ? selections[task.id]?.quantity : undefined}
            />
          ))}
        </div>

        <button onClick={onBack} className="flex items-center gap-2 text-[#26FF1B] font-medium mb-8 hover:underline">
            ...
            <div className="space-y-3">
              {freeIncludedTasks.map((task) => (
                <div key={task.id} className="flex items-start justify-between gap-4">
                  <span className="text-sm leading-tight text-white">{task.name}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="sticky bottom-0 bg-background border-t border-border px-4 py-4 max-w-[600px] mx-auto w-full">
        <div className="space-y-4 mb-4">
          <div className="flex items-center justify-between">
            ...
            <span className="font-bold text-xl">{formatPrice(totals.totalPrice)}</span>
          </div>

          <div className="flex items-center justify-between">
            <span className="font-semibold text-lg">Tiempo estimado</span>
            <span className="font-bold text-xl">{formatTime(totals.totalHours)}</span>
          </div>
...
function SummaryItem({
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      <span className="text-sm font-medium tabular-nums flex-shrink-0 text-white">{formatPrice(totalPrice)}</span>
    </div>
  )
}
